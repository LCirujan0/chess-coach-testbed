...
**Current version:** v0.82 LIVE on prod (2026-06-11, commit `b1d1980`). The full **v0.78→v0.82** batch shipped: **cross-device Supabase sync**, the **gated onboarding flow**, per-user **coach memory** + session debrief, Chess.com **insights enrichment**, per-type help, the owner QA batches (brand polish, em-dash rule, Board Vision overhaul, mobile fixes), **Calculation drills** (Spec 25), **all-type Today blocks** (warm-up rotation + SRS-due openings), universal progression on Insights, and the **strict type scale** (rule 15). Per-version detail in `docs/learnings.md`; strategy in `../docs/super-app-roadmap.md` "Roadmap v4".  
